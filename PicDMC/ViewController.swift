...
class ViewController: UIViewController, UINavigationControllerDelegate, CLLocationManagerDelegate {
    ...
    func updateForForecast(forecast: Forecast) {
        ...
        self.getAddress(point: coordinate1) { (address:String?) in
            ...
            DispatchQueue.main.async(execute: {
                ...
                var creationDateStr = ""
                if (self.picCreationDate != nil) {
                    creationDateStr = dateFormatter.string(from: self.picCreationDate!)
                    print("pic date str: ", creationDateStr)
                }
                ...
                if (UserDefaults.labels != "no") {
                    ...
                    self.creationDateLabel.text = "created \(creationDateStr)"
                } else {
                    self.tempLabel.text = "\(summary) \(intTemp)"
                    self.timeLabel.text = "\(convertedTime)"
                    self.addressLabel.text = "\(address!)"
                    self.creationDateLabel.text = "          \(creationDateStr)"
                }
    ...
    @IBAction func others(_ sender: Any) {
        ...
        // having only the image in activity items array let's instagram be
        // picked up in the list of options
    ...
    /*
    UIGraphicsBeginImageContext(view.frame.size)
    screenImage?.draw(at: CGPoint(x: -25, y: -100))
    let croppedImage = UIGraphicsGetImageFromCurrentImageContext()
    UIGraphicsEndImageContext()
        
    UIImageWriteToSavedPhotosAlbum(croppedImage!, nil, nil, nil)
    
         let shareVC = UIActivityViewController(activityItems: [imgView.image!], applicationActivities: [])
    */
    let shareVC = UIActivityViewController(activityItems: [screenImage!], applicationActivities: [])
    //shareVC.excludedActivityTypes = [UIActivityType.]
...
extension ViewController: UIImagePickerControllerDelegate {
    ...
    func setFontColor(color: String)  {
        ...
        if (color == "black") {
            ...
            self.creationDateLabel.textColor = UIColor.black
            let imgName = "share-black-64"
            let image1 = UIImage(named: "\(imgName).png")!
            self.shareBtn.setImage(image1, for: .normal)
        } else {
            ...
            self.creationDateLabel.textColor = UIColor.white
            let imgName = "share-white-64"
            let image1 = UIImage(named: "\(imgName).png")!
            self.shareBtn.setImage(image1, for: .normal)
        }
    }
